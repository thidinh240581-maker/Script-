...
local Tabs = {}
Tabs.Main = Window.Tab(Window, {Title = "Main", Icon = "eye", Desc = "DIO HUB"})
Tabs.Misc = Window.Tab(Window, {Title = "Misc", Icon = "sparkles", Desc = "DIO HUB"})
Tabs.Fight = Window.Tab(Window, {Title = "Fighting", Icon = "sword", Desc = "DIO HUB"})
Tabs.Tech = Window.Tab(Window, {Title = "Tech", Icon = "wrench", Desc = "DIO HUB"})
Tabs.Lag = Window.Tab(Window, {Title = "Lag-Ping", Icon = "leaf", Desc = "DIO HUB"})
Tabs.Anim = Window.Tab(Window, {Title = "Animations", Icon = "package", Desc = "DIO HUB"})
Tabs.Place = Window.Tab(Window, {Title = "Places", Icon = "map", Desc = "DIO HUB"})
Tabs.Moveset = Window.Tab(Window, {Title = "Moveset", Icon = "star", Desc = "DIO HUB"})
Tabs.Fling = Window.Tab(Window, {Title = "Fling", Icon = "user", Desc = "DIO HUB"})
Tabs.Info = Window.Tab(Window, {Title = "Information", Icon = "badge-info", Desc = "DIO HUB"})
...
-- Auto Farm Lowest Health
local HealthLowAttack = 15
local AutoKillLowestHealthConn = nil

-- Auto Farm Nearest
local AutoKillNearestConn = nil

-- M1 Click Reach
local M1ReachConn = nil
...
local function OnCharacterAdded(newCharacter)
    ...
    -- FIX: reset IsVoiding so Auto Void works again after death
    IsVoiding = false
    ...
    -- Also reconnect on Humanoid in case Animator wasn't ready yet
...
Tabs.Main:Dropdown({
    ...
    Values = {"Map", "Pixel", "Void", "Darkness", "Mountain", "Counter", "Atomic Base", "Atomic Base Up", "Atomic Slash", "Prison"},
    Value = "Void",
    Multi = false,
...
-- Anti-Death Counter: detect Counter tag appearing on character → teleport void 0.1s → wait 3s → fixcam → safe zone
local AntiDeathCounterActive = false
local AntiDCHandling = false -- prevent double-trigger

local function StartAntiDeathCounter()
    AntiDeathCounterActive = true
    task.spawn(function()
        local voidPos  = CFrame.new(150, -495, 30)
        local safePos  = CFrame.new(150, 705, 30)

        while AntiDeathCounterActive do
            task.wait(0.1)
            local lp   = game:GetService("Players").LocalPlayer
            local char = lp.Character
            if char and char:FindFirstChild("HumanoidRootPart") and not AntiDCHandling then
                -- Detect if the live model has a "Counter" child (death counter active)
                local liveModel = workspace.Live:FindFirstChild(lp.Name)
                local hasCounter = liveModel and liveModel:FindFirstChild("Counter") ~= nil
                if hasCounter then
                    AntiDCHandling = true
                    task.spawn(function()
                        -- 0.1s before teleporting
                        task.wait(0.1)
                        ...
                        if root2 then
                            root2.CFrame = safePos
                        end

                        -- Wait until counter is gone before resetting flag
                        local timeout = tick()
                        while tick() - timeout < 10 do
                            local lm = workspace.Live:FindFirstChild(lp.Name)
                            if not (lm and lm:FindFirstChild("Counter")) then break end
                            task.wait(0.3)
                        end
                        AntiDCHandling = false
                    end)
                end
            end
        end
...
Tabs.Main:Button({
    Title = "DIO HUB Mini",
    Desc = "First version of the DIO HUB but rly usefull",
    Locked = false,
    Callback = function()
...
-- FPS/Ping draggable button (created/destroyed by the toggle below)
local FpsPingGui = nil
local FpsPingLabel = nil
local FpsPingConn = nil

local function CreateFpsPingGui()
    if FpsPingGui then return end -- already exists
    local RunSvc  = game:GetService("RunService")
    local StatSvc = game:GetService("Stats")

    FpsPingGui = Instance.new("ScreenGui")
    FpsPingGui.Name  = "DIOHub_FPS_Ping"
    FpsPingGui.ResetOnSpawn = false
    FpsPingGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    FpsPingGui.Parent = (gethui and gethui()) or game.CoreGui

    local frame = Instance.new("Frame")
    frame.Name   = "DragFrame"
    frame.Size   = UDim2.new(0, 153, 0, 26)  -- ~15% smaller than 180×30
    ...
    FpsPingLabel.Size = UDim2.new(1, -4, 1, 0)
    FpsPingLabel.Position = UDim2.new(0, 4, 0, 0)
    FpsPingLabel.BackgroundTransparency = 1
    FpsPingLabel.TextColor3 = Color3.new(1, 1, 1)
    FpsPingLabel.Font = Enum.Font.Code
    FpsPingLabel.TextSize = 15
    FpsPingLabel.Text = "FPS: ...  Ping: ..."
    ...
    local frames = 0
    local lastTime = tick()
    FpsPingConn = RunSvc.RenderStepped:Connect(function()
        frames = frames + 1
        if tick() - lastTime >= 1 then
            local ping = 0
            pcall(function()
                ping = math.floor(StatSvc.Network.ServerStatsItem["Data Ping"]:GetValue())
            end)
            FpsPingLabel.Text = "FPS: " .. frames .. " | Ping: " .. ping
            frames   = 0
            lastTime = tick()
        end
    end)
...
local function DestroyFpsPingGui()
...
-- No Stun indicator button (draggable, shows/hides via toggle below)
local NoStunGui = nil

local function CreateNoStunGui()
    if NoStunGui then return end
    NoStunGui = Instance.new("ScreenGui")
    NoStunGui.Name = "DIOHub_NoStun_Btn"
    NoStunGui.ResetOnSpawn = false
    NoStunGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
    NoStunGui.Parent = (gethui and gethui()) or game.CoreGui
    ...
    label.TextSize = 14
    label.Text = "✔ No Stun ON"
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.Parent = frame

    -- Draggable logic
    local dragging2, dragStart2, frameStart2 = false, nil, nil
    frame.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            dragging2  = true
            dragStart2 = inp.Position
            frameStart2 = frame.Position
        end
    end)
    frame.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
    ...
    game:GetService("UserInputService").InputChanged:Connect(function(inp)
        if dragging2 and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
            ...
            frame.Position = UDim2.new(frameStart2.X.Scale, frameStart2.X.Offset + delta.X, frameStart2.Y.Scale, frameStart2.Y.Offset + delta.Y)
        end
    end)
end

local function DestroyNoStunGui()
    if NoStunGui then NoStunGui:Destroy() NoStunGui = nil end
end

Tabs.Main:Toggle({
    Title = "Button On/Off (No Stun Indicator)",
    Value = false,
    Callback = function(state)
        if state then
            CreateNoStunGui()
...
Tabs.Misc:Dropdown({
    ...
    Values = {"Map", "SafeZone1", "SafeZone2", "Pixel", "Void", "DarkNess", "Montain", "Counter", "Counter Up", "Atomic Base", "Atomic Base Up", "Atomic Slash", "Prison"},
    Multi = false,
    AllowNone = false,
    Callback = function(selected)
        local toolData = {
...
Tabs.Fight:Button({
    ...
    Callback = function()
        ...
        loadstring(game:HttpGet("https://raw.githubusercontent.com/DiosDi/DIO HUB/refs/heads/main/TrashCan-Farm"))()
    end
})
...
Tabs.Fight:Toggle({
    ...
    Callback = function(State)
        ...
        -- Gửi thông báo khi Bật/Tắt
        game.StarterGui:SetCore("SendNotification", {
            Title = "DIO HUB",
            Text = State and "Auto Get Emote Activated." or "Auto Get Emote Deactivated.",
            Icon = "http://www.roblox.com/asset/?id=84519376661277",
            Duration = 5
        })

        -- Ghép vòng lặp thẳng vào đây: Chỉ chạy khi bật
        if State then
            task.spawn(function()
...
Tabs.Fight:Toggle({
    ...
    Default = false,
    Callback = function(State)
        _G.AutoHopServer = State
        
        if not State then
            _G.HopTimer = nil -- Reset thời gian khi Tắt
        else
            -- Ghép vòng lặp thẳng vào đây: Chỉ chạy đếm giờ khi Bật
            task.spawn(function()
                while _G.AutoHopServer do
                    pcall(function()
                        ...
                        if tick() - _G.HopTimer >= _G.HopDelay then
                            _G.HopTimer = tick() -- Reset mốc thời gian trước khi out

                            local queue_on_teleport = queue_on_teleport or (syn and syn.queue_on_teleport) or (fluxus and fluxus.queue_on_teleport)
                            if queue_on_teleport then
                                queue_on_teleport("loadstring(game:HttpGet('https://pastefy.app/6hROay1y/raw'))()")
                            end

                            game:GetService("TeleportService"):Teleport(game.PlaceId, game:GetService("Players").LocalPlayer)
                        end
                    end)
                    task.wait(1) -- Check thời gian mỗi giây
                end
            end)
        end
    end
...
Tabs.Fight:Slider({
    ...
    Value = {Min = 10, Max = 120, Default = 1},
    Callback = function(Value)
        _G.HopDelay = Value * 60 -- Đổi phút thành giây
    end
})
Tabs.Fight:Toggle({
    Title = "Instant Kill",
    Value = false,
    Callback = function(state)
        getgenv().InstantKillEnabled = state
        if state then
            task.spawn(function()
                while getgenv().InstantKillEnabled do
                    pcall(function()
                        local char = LocalPlayer.Character
                        ...
                        if myRoot then
                            for _, model in pairs(workspace.Live:GetChildren()) do
                                ...
                                if hum and root and model ~= char and hum.Health > 0 then
...
Tabs.Fight:Toggle({
    ...
    Callback = function(state)
        ...
        local function UpdateRootPart()
            ...
            if not char then return end
            myRootPart = char:FindFirstChild("HumanoidRootPart")
            if not myRootPart then
                myRootPart = char:WaitForChild("HumanoidRootPart", 5)
            end
        end
        ...
        -- Check if a target character currently has Death Counter active
        local function TargetHasDeathCounter(targetChar)
            if not targetChar then return false end
            -- Check the Live folder model for the Counter tag
            local liveModel = workspace.Live:FindFirstChild(targetChar.Name)
            return liveModel ~= nil and liveModel:FindFirstChild("Counter") ~= nil
        end

        -- Find the player with the lowest health that is at or below HealthLowAttack threshold
        local function FindLowestHealthTarget()
            ...
            for _, model in pairs(game.Workspace.Live:GetChildren()) do
                if IsPlayer(model) then
                    ...
                    if humanoid and rootPart
                        and model ~= Player.Character
                        and humanoid.Health > 0
                        and humanoid.Health <= HealthLowAttack
                        and humanoid.Health < lowestHP
                    then
        ...
        -- FIX: Disconnect previous CharacterAdded connection to prevent accumulation
        if AutoKillLowestHealthConn then
            AutoKillLowestHealthConn:Disconnect()
            AutoKillLowestHealthConn = nil
        end
                ...
                pcall(function()
                    ...
                    if myRootPart then
                        ...
                        if target then
                            ...
                            -- Skip if target has Death Counter active
                            if TargetHasDeathCounter(targetChar) then
                                return
                            end
                            ...
                            Player.Character:SetPrimaryPartCFrame(CFrame.new(
                                ...
                                - target.CFrame.LookVector * getgenv().TeleportDistance
                                + Vector3.new(0, -6, 0),
                                target.Position - Vector3.new(0, target.Size.Y / 2, 0)
                            ))

                            local targetLiveModel = workspace.Live:FindFirstChild(targetChar.Name)
                            if targetLiveModel
                                and not targetLiveModel:FindFirstChild("RagdollSim")
                                and not targetLiveModel:FindFirstChild("AbsoluteImmortal")
                            then
                                task.spawn(function()
        ...
        -- FIX: One clean connection; update root after respawn so loop resumes correctly
        AutoKillLowestHealthConn = Player.CharacterAdded:Connect(function()
            task.wait(1.5)
            UpdateRootPart()
        end)
    end
...
Tabs.Fight:Slider({
    Title = "Heath Low Will Attack",
    Value = {Min = 1, Max = 100, Default = 15},
    Callback = function(value)
        HealthLowAttack = value
    end
})
...
Tabs.Fight:Toggle({
    ...
    Callback = function(state)
        ...
        -- FIX: Disconnect previous connection to prevent accumulation
        if AutoKillNearestConn then
            AutoKillNearestConn:Disconnect()
            AutoKillNearestConn = nil
        end
            ...
            while getgenv().AutoKillNearestPlr == true do
                pcall(function()
                    ...
                    if target then
                        ...
                        local targetLiveModel = workspace.Live:FindFirstChild(target.Parent.Name)
                        if targetLiveModel
                            and not targetLiveModel:FindFirstChild("RagdollSim")
                            and not targetLiveModel:FindFirstChild("AbsoluteImmortal")
                        then
        ...
        -- FIX: Store connection so it can be cleaned up on next toggle
        AutoKillNearestConn = Player.CharacterAdded:Connect(function()
            task.wait(1.5)
            UpdateRootPart()
        end)
    end
...
-- ===== AUTO KILL ALL (SKILL 2 - TWIN FANGS) =====
local AutoSkill2Enabled = false
local AutoSkill2Conn = nil

local Skill2AnimGather1 = "rbxassetid://18896229321" -- anim 1: gather
local Skill2AnimGather2 = "rbxassetid://7807831448"  -- anim 2: gather + teleport void at end
local Skill2AnimEnd     = "rbxassetid://14516273501" -- anim 3: finish

local Skill2VoidPos = CFrame.new(150, -495, 30)
local Skill2MapPos  = CFrame.new(139, 440, 32)

local function GetTwinFangs()
    local lp = game:GetService("Players").LocalPlayer
    -- Check backpack
    local bp = lp.Backpack:FindFirstChild("Twin Fangs")
    if bp then return bp end
    -- Check character
    local ch = lp.Character
    if ch then
        local ct = ch:FindFirstChild("Twin Fangs")
        if ct then return ct end
    end
    -- Check nil instances
    for _, v in ipairs(getnilinstances and getnilinstances() or {}) do
        if v:IsA("Tool") and v.Name == "Twin Fangs" then return v end
    end
    return nil
end

local function IsPlayingAnim(animId)
...
local function WaitUntilAnimNearEnd(animId, threshold)
    threshold = threshold or 0.85
    while true do
        local playing, track = IsPlayingAnim(animId)
        if not playing then break end
        local len = track.Length
        local pos = track.TimePosition
        if len > 0 and (pos / len) >= threshold then break end
        task.wait(0.05)
    end
end

local function RunAutoSkill2()
    while AutoSkill2Enabled do
        pcall(function()
            local lp = game:GetService("Players").LocalPlayer
            local char = lp.Character
            if not char then task.wait(1) return end
            local communicate = char:FindFirstChild("Communicate")
            if not communicate then task.wait(0.5) return end

            -- Step 1: Activate UTL (G key press)
            ...
            -- Step 2: Activate Twin Fangs (Console Move)
            local twinFangs = GetTwinFangs()
            if twinFangs then
                communicate:FireServer(unpack({{
                    ["Tool"] = twinFangs,
                    ["Goal"] = "Console Move",
                ...
                -- Step 3: Wait through anim 1 (gather phase)
                local gatherTimeout = tick()
                while not IsPlayingAnim(Skill2AnimGather1) and tick() - gatherTimeout < 3 do
                    task.wait(0.05)
                end
                -- Wait anim 1 to finish playing (gather all)
                while IsPlayingAnim(Skill2AnimGather1) do
                    task.wait(0.05)
                end

                -- Step 4: Anim 2 starts — wait until ~55% (mid-point = gathered enough), then at 85%+ teleport void
                local anim2Timeout = tick()
                while not IsPlayingAnim(Skill2AnimGather2) and tick() - anim2Timeout < 2 do
                    task.wait(0.05)
                end
                WaitUntilAnimNearEnd(Skill2AnimGather2, 0.85)

                -- Teleport to void position
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    root.CFrame = Skill2VoidPos
                end
                task.wait(0.25) -- wait 0.25s in void

                -- Auto Use End
                local twinFangsNil = GetTwinFangs()
                if twinFangsNil then
                    communicate:FireServer(unpack({{
                        ["Goal"] = "Auto Use End",
                        ["Tool"] = twinFangsNil
                    }}))
                ...
                -- Step 5: Wait for anim 3 to start
                local anim3Timeout = tick()
                while not IsPlayingAnim(Skill2AnimEnd) and tick() - anim3Timeout < 3 do
                    task.wait(0.05)
                end
                -- Wait anim 3 near end (85%) then teleport back
                WaitUntilAnimNearEnd(Skill2AnimEnd, 0.85)
                root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    root.CFrame = Skill2MapPos
                end
            end

            -- Wait for skill cooldown (~8s typical) before looping
            task.wait(8)
        end)
    end
end
...
Tabs.Fight:Toggle({
    Title = "Auto Kill all (Skill 2 - Twin Fangs)",
    Value = false,
    Callback = function(state)
        AutoSkill2Enabled = state
        if state then
            task.spawn(RunAutoSkill2)
        end
    end
})
-- ===== END AUTO KILL ALL SKILL 2 =====
    ...
    Callback = function(state)
        ...
        task.spawn(function()
            while getgenv().AutoTeleportPunching == true do
                pcall(function()
                    UpdateRootPart()
                    local liveChar = workspace.Live:FindFirstChild(game.Players.LocalPlayer.Name)
                    local isPunching = liveChar and liveChar:FindFirstChild("M1ing")
        ...
        -- FIX: Store connection so it can be cleaned up on next toggle
        M1ReachConn = Player.CharacterAdded:Connect(function()
            task.wait(3)
            UpdateRootPart()
        end)
    end
...
Tabs.Fight:Input({
    ...
    Callback = function(name)
        if not name or name == "" then
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {Title = "DIO HUB", Text = "Fight target cleared", Duration = 3})
            return
        end
        ...
        for _, target in ipairs(Players:GetPlayers()) do
            if target ~= LocalPlayer and (string.find(string.lower(target.Name), lowerName, 1, true) or string.find(string.lower(target.DisplayName), lowerName, 1, true)) then
                ...
                game:GetService("StarterGui"):SetCore("SendNotification", {Title = "DIO HUB", Text = "Fight target: " .. target.DisplayName, Duration = 3})
                return
            end
        end
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "DIO HUB", Text = "Player not found", Duration = 3})
    end
})
    ...
    if FlingStates.notifyOnDeath and SelectedPlayer then
        ...
        if humanoid then
            DeathNotifyConnection = humanoid.Died:Connect(function()
                SendNotification("DIO HUB", SelectedPlayer.DisplayName .. " Died")
            end)
        end
    end
end
...
local function SelectPlayer(playerName)
    ...
    if not playerName or playerName == "" then
        SendNotification("DIO HUB", "No One Selected")
        return
    end
    ...
    if foundPlayer then
        ...
        SendNotification("DIO HUB", "Selected: " .. foundPlayer.DisplayName, "https://www.roblox.com/headshot-thumbnail/image?userId=" .. foundPlayer.UserId .. "&width=420&height=420&format=png", 10)
        SetupDeathNotification()
    else
        SendNotification("DIO HUB", "Player Not Found...")
    end
end
...
Tabs.Fling:Button({
    ...
    Callback = function()
        setclipboard("https://discord.gg/MjGt2bTWBS")
        SendNotification("DIO HUB", "Discord link copied to clipboard!")
    end
})
    ...
    Callback = function()
        if SelectedPlayer then
            ...
            if myRoot and theirRoot then
                ...
                SendNotification("DIO HUB", "Could not find character to teleport.")
            end
        else
            SendNotification("DIO HUB", "No One Selected")
        end
    ...
    Callback = function()
        if SelectedPlayer then
            ...
            if targetHumanoid then
                ...
                SendNotification("DIO HUB", "Could not find the player's character to view.")
            end
        else
            SendNotification("DIO HUB", "No One Selected")
        end
...
PlayersService.PlayerRemoving:Connect(function(player)
    if SelectedPlayer and player == SelectedPlayer then
        SendNotification("DIO HUB", player.DisplayName .. " left the game")
        SelectedPlayer = nil
...
local MovesetScripts = {
    {"The Garbage Ghost (Universal)", "https://raw.githubusercontent.com/DiosDi/DIO HUB/refs/heads/main/TheGarbageGhost"},
    ...
    Buttons = { 
        {
            ...
            Title = "Copy Link",
            Callback = function()
                setclipboard("https://discord.gg/MjGt2bTWBSE")
            end
        }
    }
})
