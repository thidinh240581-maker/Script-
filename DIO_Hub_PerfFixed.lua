...
-- ============================================================
-- [GHOST MOB FIX] Helper dùng chung: Bone / Cake / Level
-- Cách hoạt động:
--   Gọi __DIO_GhostTrack() ở đầu mỗi inner kill-loop.
--   Gọi __DIO_GhostCheck(mob) SAU mỗi lần Attack.Kill/Kill2.
--   Trả về: false = bình thường | "skill" = đang bắn skill | true = ghost 6s, chuyển mob
--
-- Logic:
--   0-3s HP không đổi  → vẫn Attack bình thường (chờ xem)
--   3s   HP không đổi  → bắn Z,X,C (Melee/Fruit) hoặc Z,X (Sword)
--   6s   HP không đổi  → trả true, vòng lặp ngoài chuyển sang spawn/mob khác
--   HP đổi bất kỳ lúc → reset timer (mob không còn ghost)
-- ============================================================
local __DIO_GhostHP    = nil
local __DIO_GhostT     = nil
local __DIO_GhostSkill = false

local function __DIO_GhostTrack()  -- Gọi 1 lần khi bắt đầu attack mob mới
    __DIO_GhostHP    = nil
    __DIO_GhostT     = nil
    __DIO_GhostSkill = false
end
...
local function __DIO_GhostCheck(mob)
    ...
    if __DIO_GhostHP == nil then
        __DIO_GhostHP = curHp
        __DIO_GhostT  = tick()
        return false
    end

    if curHp ~= __DIO_GhostHP then
        -- HP đổi → mob nhận damage, reset timer
        __DIO_GhostHP    = curHp
        __DIO_GhostT     = tick()
        __DIO_GhostSkill = false
        return false
    ...
    local elapsed = tick() - __DIO_GhostT

    if elapsed >= 6 then
        -- 6s không gây được damage → chuyển mob/bãi farm
        __DIO_GhostTrack()
        return true   -- caller break và tìm mob khác
    end

    if elapsed >= 3 and not __DIO_GhostSkill then
        -- 3s không gây damage → bắn skill phá ghost
        __DIO_GhostSkill = true
        pcall(function()
            EquipWeapon(_G.SelectWeapon)
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- [FIX FARM-LEVEL LOOP] Ban cu: __DIO_wait(2) sau moi mob = 2s chet liet.
                    -- Ghost fix: neu 3s HP khong doi -> ban skill; 6s -> bo mob sang cai khac.
                    -- Guard Root stale: kiem tra v.Parent truoc moi tp tranh lua error khi mob bi xoa.
                    local foundMob = false
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and Attack.Alive(v) then
                            foundMob = true
                            __DIO_GhostTrack() -- reset ghost timer cho mob moi
                            repeat
                                __DIO_wait(Sec)
                                if not v.Parent or not v:FindFirstChild("HumanoidRootPart") then break end
                                -- Cap nhat Root theo frame (tranh stale sau respawn)
                                ...
                                -- Ghost check: 3s skill / 6s doi mob
                                if __DIO_GhostCheck(v) then
                                    foundMob = false -- force tim mob khac
                                    break
                                end
                            until not _G.Level or not v.Parent or v.Humanoid.Health <= 0
                            -- [FIX] Bo __DIO_wait(2) chet liet - loop chinh se poll Sec=0.25s la du
                        end
                    end
    ...
    while __DIO_wait(1) do -- [FIX PERF 62] 0.1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        if _G.Auto_Cake_Prince then
            pcall(function()
                ...
                if bestMob then
                    -- [FIX CAKE LAG] Ban cu: repeat task.wait() = 60 lan/s moi lan goi _tp + Attack.Kill2
                    -- = ~180 heavy function call/s -> chinh nguyen nhan 10-20fps.
                    -- Fix: __DIO_wait(Sec) = 4/s du attack, giam 93% CPU, + ghost fix.
                    __DIO_GhostTrack()
                    repeat
                        __DIO_wait(Sec)
                        if not bestMob.Parent or bestMob.Humanoid.Health <= 0 then break end
                        -- Cache mirror check 1 lan/loop thay vi truy cap workspace moi frame
